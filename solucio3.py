...
"""
Exercici 4: Mostrar quants estudiants hi ha matriculats de cada matèria
Nom de la funció: mostrar_estudiants_matèries
Paràmetres:
notes: Un diccionari on les claus són noms d'estudiants i els valors són les seves notes (un altre diccionari amb matèries com a claus i notes com a valors).
Retorna:
None
...
Defineix la funció, i fes un exemple de crida amb la variable notes_estudiants, 
Pots utilitzar la funció comptar_estudiants_materia
Passos:
Primer recorre el diccionari per tal de fer un conjunt de matèries
Recorre aquest diccionari per fer crides a la funció comptar_estudiants_matèria i mostra el missatge corresponent
La sortida  per consola de la crida hauria de ser: 
Hi ha 3 estudiant(s) matriculat(s) a la matèria Matemàtiques.
Hi ha 2 estudiant(s) matriculat(s) a la matèria Història.
Hi ha 1 estudiant(s) matriculat(s) a la matèria Ciències.

"""
def mostrar_estudiants_matèries(notes):
    matèries = set()
    for estudiant in notes:
        matèries.update(notes[estudiant].keys())
    for matèria in matèries:    
        print(f"Hi ha {comptar_estudiants_matèria(notes, matèria)} estudiant(s) matriculat(s) a la matèria {matèria}.")
